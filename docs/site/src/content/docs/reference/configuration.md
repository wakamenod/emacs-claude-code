...
| `ecc-jev-enabled` | `nil` | When non-nil, what each finished turn meant becomes the mark opening that session's row in the sidebar: waiting on a decision, blocked, or stopped part way. Requires [jev.el](https://github.com/wakamenod/jev.el) and `(require 'ecc-jev)`, and sends the last assistant message of every finished turn to TypeSafe AI, which is charged for. See [Spaces and worktrees](/emacs-claude-code/features/spaces/#what-an-idle-session-meant) |
