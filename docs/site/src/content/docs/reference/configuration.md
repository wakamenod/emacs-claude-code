...
| `ecc-layout` | `'classic` | How the session windows are laid out. `'classic` puts a transcript in a side window with a role -- main, sub-1, sub-2 -- and deals those roles out again for one project; `'spaces` gives every project a tab of the tab bar and leaves the windows inside it alone. See [Spaces and worktrees](/emacs-claude-code/features/spaces/) |
| `ecc-worktree-directory` | `".claude/worktrees"` | Where `ecc-start-worktree` puts a checkout. A relative name hangs off the repository; an absolute one is a directory every repository shares, and a checkout lands at `<directory>/<repository>/<branch-slug>` |
