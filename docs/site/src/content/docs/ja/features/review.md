...
`ecc-review` が対象にするのは、そのセッションが編集・作成したファイルだけです。自分の手で書いた変更 — Claude に何か頼む前から進んでいた作業 — をレビューするには、Transient メニューで `G` を押すか、`C-c c G`、または `M-x ecc-review-worktree` を実行します。

セッションのプロジェクトのリポジトリ全体を `HEAD` と比較するので、ステージ済みかどうかによらず未コミットの変更がすべて表示され、さらに Git が未追跡のファイル（`.gitignore` の除外対象は除く）も並びます。未追跡のバイナリファイルや、`ecc-review-untracked-max-bytes` を超える大きさのファイルは、中身ではなくファイル名だけが出ます。プレフィクス引数（`C-u G`）を渡すと比較対象を指定できます。`HEAD` のようなリビジョン、ブランチ全体なら `main...HEAD` のような範囲、空のままならステージされていない分だけです。

対象のプロジェクトは、コマンドを実行したバッファのプロジェクトです（たまたまカレントになっているセッションのものではありません）。別プロジェクトで動いているセッションに渡してしまえば、Claude は見てもいないファイルを直すよう指示されることになるからです。コメントの送り先はそのプロジェクトのセッションで、まだ無ければ起動するか尋ねます。実際これが通常の入り口で、作業していたプロジェクトのファイル上で `C-c c G` を押して yes と答えるだけです。

コメントはハンク単位で付くので、レビューは git に文脈行を要求しません。変更行のかたまりごとに 1 ハンクへ分かれ、コメントは対象の行だけを連れていきます。前後を読みながら見たい場合は `ecc-review-context-lines` を上げてください（変数なので `(setq ecc-review-context-lines 3)`）。git には `-U` として渡されます。これはレビューのために ecc が起動する git の引数であって、`git config` を読み書きするものではありません。コマンドラインの git には影響しません。なお、単一の提案のレビューは別扱いで、常に前後 3 行が残ります（`ecc-review-proposal-context-lines`）。何を上書きしようとしているかは、その提案を許可するかどうかの判断の半分だからです。

あとは上と同じで、ハンクにコメントを付けて `C-c C-c` を押せば、1 つのプロンプトとしてセッションに送られます。2 つのレビューは別のバッファなので、作業ツリーのレビューがセッションの変更のレビューを置き換えることはありません。
