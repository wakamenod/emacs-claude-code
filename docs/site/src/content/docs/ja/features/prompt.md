...
`M-p` はプロンプト領域全体を置き換えるため、古い履歴を遡るのには向きません。`C-c C-r`（`ecc-prompt-history-insert`）は履歴を新しい順に 1 件 1 行の補完候補として表示し、選んだプロンプトを全文カーソル位置に挿入します。書きかけの文章はそのまま残ります。
...
`/resume` は、このプロジェクトで記録された会話のどれを続けるかを尋ね、この窓をその会話に乗り換えさせます。ウィンドウもタブもバッファもセッション名もそのままで、変わるのは中身の会話だけです。ターミナルクライアントの `/resume` と同じ振る舞いですが、CLI はヘッドレスクライアントにこのコマンドを提供していないため、名前は Emacs 側のものです。

先に Emacs が動かしている CLI を停止します（実行中のターンは中断して終了を待ちます）。同じセッション ID で 2 つのプロセスが走ると、録画が黙って枝分かれするためです。すでにターンのある会話や、送信待ちのプロンプトが残っている場合は、必ず確認を挟みます。離れた会話はそのまま残り、`ecc-history-open` で読み直せます。

`/resume <session-id>` と打てば、尋ねずにその ID の録画を開きます。Space と組み合わせると、何も動いていないプロジェクトに移動すると新しいセッションが起動し、`/resume` でそこにあった会話に戻る、という流れになります。
...
画像は送信されるだけでなく、トランスクリプトにも描画されます。プロンプトに添付した画像は、その発言の下に表示されます。Claude が返した画像は、それを生成した呼び出しの下に表示されます。これには、メッセージ内の `image` ブロック、`.png` の `Read` の結果、MCP ツールのスクリーンショットが含まれます。Base64 がバッファに入ることはありません。実データはセッションの画像ディレクトリに書き出され、トランスクリプトはパスのみを保持します。

`PATH` 上に `ffmpeg` があれば、動画の先頭フレームが表示され、`RET` で外部プレイヤーで開きます。`ffmpeg` が利用できない場合は、ファイル名の行だけが表示されます。フレームの取り出しはサブプロセスで実行され、完了を待ちません。そのため、フレームが表示されるまではファイル名の行がそのまま残ります。

GIF は描画されるとすぐに動き出し、画面にある限りループします。GIF 上で `v` を押すと停止または再開します。`v` が制御するのは動きだけです。`RET` は画像を開きます。静止画は `image-mode` で、動画は外部プレイヤーで開きます。`ecc-image-inline`（メニューの `I` でも同様）は描画をオフにし、ファイル名の行だけを残します。描画する画像は高さが制限され、幅は `ecc-chat-text-width` に従います。
