...
GIF は表示されるとすぐにアニメーションを開始し、画面内にある限りループします。GIF 上で `v` を押すと停止または再開します。`v` が制御するのはアニメーションのみです。`RET` はファイルを開きます（静止画は `image-mode`、動画は外部プレイヤー）。`ecc-image-inline`（メニューの `I`）は画像表示をオフにし、ファイル名を示す行だけを残します。表示される画像は高さが制限され、幅は `ecc-chat-text-width` に従います。
