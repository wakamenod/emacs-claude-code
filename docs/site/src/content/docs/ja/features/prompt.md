...
`/resume` は、このプロジェクトで記録されたどの会話を続けるかを尋ね、現在のウィンドウをその会話に切り替えます。ウィンドウ、タブ、バッファ、セッション名はそのままで、切り替わるのは中身の会話だけです。これはターミナルクライアントの `/resume` と同じ動作です。CLI はヘッドレスクライアント向けにこのコマンドを一覧していないため、名前は Emacs 独自のものです。
