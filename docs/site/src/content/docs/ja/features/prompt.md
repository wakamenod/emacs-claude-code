...
| `C-c C-r` | 履歴から選んでカーソル位置に挿入 |
| `C-c C-s` | CLI からの提案プロンプトを採用 |

プロンプトの履歴は全セッションで共有されるため、あるセッションで入力したプロンプトを別のセッションでもすぐに再利用できます。

`M-p` はプロンプト領域全体を置き換えるため、古い履歴を遡るのには向きません。`C-c C-r`（`ecc-prompt-history-insert`）は履歴を新しい順に 1 件 1 行の補完候補として表示し、選んだプロンプトの全文をカーソル位置に挿入します。書きかけの文章はそのまま残ります。
...
| `C-c C-z` | 実行中のターンを中断（`C-c C-g` は意図的に未割り当て。他のモードと同じくプレフィクスの取り消しになる） |
...
CLI の一覧に含まれないコマンドのうち、`/btw`、`/hooks`、`/plugins`、`/login`、`/logout`、`/auth-status`、`/resume` は Emacs 側で処理され、モデルには送信されません。
...
`/resume` は、このプロジェクトで記録されたどの会話を続けるかを尋ね、現在のウィンドウをその会話に切り替えます。ウィンドウ、タブ、バッファ、セッション名はそのままで、切り替わるのは中身の会話だけです。これはターミナルクライアントの `/resume` と同じ動作です。CLI はヘッドレスクライアントにこのコマンドを提供していないため、ここでの `/resume` は ecc 独自の実装です。

Emacs はまず実行中の CLI プロセスを停止します（実行中のターンは中断して終了を待ちます）。同一のセッション ID で 2 つのプロセスが動くと、気付かないうちに記録が分岐してしまうためです。会話にすでにターンがある場合や、送信待ちのプロンプトがある場合は、離れる前に確認を求めます。離れた会話は保持され、`ecc-history-open` で再度開けます。

`/resume <session-id>` は確認なしに指定した ID の記録を再開します。これは Space との連携に便利です。何も動いていないプロジェクトを開くと新規セッションが始まりますが、`/resume` を使えば以前の会話に戻れます。
...
画像は送信されるだけでなく、トランスクリプトにも表示されます。プロンプトに添付した画像は、指示を出したプロンプト帯の下に表示されます。Claude が返した画像は、それを生成した呼び出しの下に表示されます。これには、メッセージ内の `image` ブロック、`.png` の `Read` の結果、MCP ツールのスクリーンショットが含まれます。Base64 データがバッファに入ることはありません。ペイロードはセッションの画像ディレクトリに書き出され、トランスクリプトはそのパスのみを保持します。

`PATH` 上に `ffmpeg` があれば、動画の先頭フレームが表示され、`RET` を押すと外部プレイヤーで動画が開きます。`ffmpeg` が利用できない場合は、ファイル名を示す行だけが表示されます。フレームの取り出しはサブプロセスで非同期に実行されるため、準備ができるまではファイル名の行がそのまま残ります。

GIF は表示されるとすぐにアニメーションを開始し、画面内にある限りループします。GIF 上で `I` を押すと停止または再開します。`I` が制御するのはアニメーションのみです。`RET` はファイルを開きます（静止画は `image-mode`、動画は外部プレイヤー）。`ecc-image-inline`（メニューの `I`）は画像表示をオフにし、ファイル名を示す行だけを残します。表示される画像は高さが制限され、幅は `ecc-chat-text-width` に従います。
...
| `I` | カーソル位置の GIF アニメーションを停止・再開 |
| `a` | 待機中リクエストを許可 |
| `d` | リクエストを拒否（リクエスト以外の場所では diff を開く） |
| `g` | トランスクリプトを再描画 |
| `L` | 生のプロトコルログを表示 |
| `t` / `R` | ターミナルクライアントへ引き渡し / セッション再開 |
| `C-c C-k` | プロンプト領域をクリア（プロンプト側と同じ。一つのキーはバッファ内で一つの意味） |
| `S-TAB` | 権限モードを切り替え |
