...
各項目にはセッション名、最終更新時刻、ディレクトリ（過去の記録の場合は最初のプロンプト）が表示されます。`-f` スイッチを付けると、選択した会話を直接引き継ぐのではなく、新しい会話として分岐（フォーク）して開始できます。キーから直接操作する場合は、`C-c c r` がそのまま再開、`C-u C-c c r` が同じ分岐にあたります。
...
`j` は Space への移動、`b` はサイドバーを開く、`z` はカーソルのあるウィンドウでタブを埋める（もう一度で元に戻る）、`V` はこの Space のウィンドウを新しいタブと同じ配置に整え直す、`X` はこの Space を閉じて動いているセッションを止めます。

`W` は worktree メニューを開きます。その中の `c` は worktree を作ってそこでセッションを開始、`o` は既にある worktree でセッションを開始、`k` は worktree を削除します。
