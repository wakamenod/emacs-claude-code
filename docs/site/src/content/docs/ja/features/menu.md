...
各項目には名前、最終アクティブ時刻、ディレクトリ（記録の場合は最初のプロンプト）が表示されます。`-f` スイッチを使うと、選択した会話を継続する代わりに、新しい会話として分岐します。キーバインドでは、`C-c c r` で直接再開し、`C-u C-c c r` で分岐します。
...
`j` は Space を切り替えます。`b` はサイドバーを開きます。`z` は現在のウィンドウをタブ全体に最大化（トグル）します。`V` はこの Space のウィンドウを新規タブの配置にリセットします。`X` はこの Space を閉じ、実行中の処理をすべて停止します。

`W` は worktree メニューを開きます。このメニューでは、`c` は worktree を作成してセッションを開始し、`o` は既存の worktree でセッションを開始し、`k` は worktree を削除します。
