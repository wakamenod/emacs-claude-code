...
`C-c c c`（`ecc-start`）は、現在のバッファのプロジェクトでセッションを開始します。そのバッファがファイルかディレクトリを開いていればそれを使い、そうでなければ直近にファイルかディレクトリを開いていたバッファを使います。トランスクリプト、ダッシュボード、ヘルプ、スクラッチバッファから開始した場合も、この直近のバッファに従います。プレフィックス引数（`C-u C-c c c`）を付けると、そのプロジェクトを既定値としてディレクトリの入力を求めます。
...
Transient メニューの `B`、`C-c c B`、または `M-x ecc-dashboard` でダッシュボードを開きます。
...
`k` はプロセスのみを停止して記録は残すため、後から `r` で再開できます。セッションを止めるとそのウィンドウとトランスクリプトも失われるため、実行前に確認を求めます。`D` は記録ファイル自体を完全に削除します。

`a` と `d` は、応答する前に何に応答するのかを確認します（`ecc-answer-confirm` を `nil` にすると確認を省けます）。また `ecc-answer-exclude-tools`（既定では `Bash`）に挙げられたツールには応答しません。シェルコマンドは、桁幅に切り詰められた要約からではなく、`RET` で開くトランスクリプトで読んでから答えるべきものだからです。サイドバーの `a` と `d` も同じ動作です。応答時に何が送られるかは [プロンプトとトランスクリプト](/emacs-claude-code/ja/features/prompt/) をご覧ください。
...
タブは作成順に並ぶため、位置が変わりません。タブを `mouse-1` でクリックすると、ウィンドウがそのセッションに切り替わります。セッションバッファ内では `C-c C-t`（`ecc-switch-session`）でも同様です。`x` ボタンをクリックすると、確認のうえセッションを終了します。

![セッションウィンドウが別のセッションに切り替わり、選択タブが greet から notes へ移動してトランスクリプトが置き換わる様子](../../../../assets/switch.gif)
...
ウィンドウのタブには、そのウィンドウ自身のプロジェクトのセッションだけが並びます。異なるプロジェクトのウィンドウを並べても、それぞれのタブ行は別々で、互いのセッションは現れません。すべてのセッションを 1 行に並べるには、`ecc-tab-line-scope` を `'all` に設定してください。

スコープ外のセッションにも、`C-c C-t`（`ecc-switch-session`）、ダッシュボード（`C-c c B`）、`C-c c n`（`ecc-next-attention`）でアクセスできます。タブは画面にないため、応答待ちになっても点滅はしません。モードラインの `⚠ecc:N` カウントや通知には引き続き反映されます。

タブの下にあるヘッダーラインには、左側にセッションの現在状態、その隣にプロジェクト名、右側にコンテキストウィンドウの残り容量が表示されます（容量が逼迫してくると黄色、赤色へと変化します）。

## 1 つのプロジェクトに絞る

`M-x ecc-focus-project` は、フレームの表示を 1 つのプロジェクトだけにリセットします。他のプロジェクトのトランスクリプトで埋まったフレームに対する、`classic` での解決策です。`spaces` ではプロジェクトごとにタブがあるため、このコマンドはその Space に切り替えるだけでウィンドウ構成には手を加えません。Space に小文字キーを割り当てた整理に伴い、キーバインドはなくなりました。

![2 つのプロジェクトがひとつのフレームを埋めている状態から、片方を選ぶと他方のウィンドウが消え、このプロジェクトの 2 つめのセッションがその場所に入り、左のソースも切り替わる](../../../../assets/focus.gif)

- 他のすべてのプロジェクトのセッションウィンドウが画面から消えます。`ecc-focus-project` は何も kill せず、プロセスも停止しません。その別のプロジェクトにフォーカスすればウィンドウは元に戻り、単一のセッションだけなら `C-c c v`、`C-c c n`、サイドバー、ダッシュボードから戻せます。
- 選んだプロジェクトのセッションが、最後に使った順にウィンドウの役割へ割り当てられます。最後に作業していたセッションがメインウィンドウに来ます。
- メインウィンドウがそのプロジェクトのソースに切り替わります。表示中のプロジェクトバッファ、そこで最後に編集したバッファ、プロジェクト内で最近使ったバッファ、プロジェクトルートの Dired の順に選択されます。プレフィックス引数（`C-u M-x ecc-focus-project`）を付けると、代わりにバッファの選択を求められます。
