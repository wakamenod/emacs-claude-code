...
タブはセッションが開始された順序で並ぶため、作業中に位置が動くことはありません。タブを `mouse-1` でクリックするか、セッションバッファ内で `C-c C-t`（`ecc-switch-session`）を押すと、そのウィンドウの表示対象をそのセッションに切り替えます。タブ右端の `x` をクリックすると確認のうえセッションを終了します。
...
スコープ外のセッションにも、`C-c C-t`（`ecc-switch-session`）、ダッシュボード（`C-c c B`）、`C-c c n`（`ecc-next-attention`）で到達できます。ただしタブが画面上にないため、応答待ちになっても点滅はしません。モードラインの `⚠ecc:N` と通知は変わらず知らせます。

タブの下にあるヘッダーラインには、左側にセッションの現在状態、その隣にプロジェクト名、右側にコンテキストウィンドウの残り容量が表示されます（容量が逼迫してくると黄色、赤色へと変化します）。

## 1 つのプロジェクトに絞る

`M-x ecc-focus-project` を使うと、フレームの表示を 1 つのプロジェクトに戻せます。他人のトランスクリプトで埋まったフレームに対する `classic` の答えです。`spaces` ではプロジェクトごとにタブがあるので、このコマンドはその Space へ移動するだけでウィンドウには触れません。Space に小文字キーを譲る整理の際に、専用キーはなくなりました。

![2 つのプロジェクトがひとつのフレームを埋めている状態から、片方を選ぶと他方のウィンドウが消え、このプロジェクトの 2 つめのセッションがその場所に入り、左のソースも切り替わる](../../../../assets/focus.gif)

- 他のプロジェクトのセッションウィンドウが画面から消えます。`ecc-focus-project` は何も kill せず、プロセスも止めません。そのプロジェクトに絞り直せばウィンドウは戻り、1 つのセッションだけなら `C-c c v`、`C-c c n`、サイドバー、ダッシュボードから戻せます。
- 選んだプロジェクトのセッションが、最後に使った順にウィンドウの役割へ割り当てられます。最後に作業していたセッションがメインウィンドウに来ます。
- メインウィンドウがそのプロジェクトのソースに切り替わります。画面にあるそのプロジェクトのバッファ、そのプロジェクトで最後に作業したバッファ、最近使ったそのプロジェクトのバッファ、ルートの Dired の順に選ばれます。プレフィックス引数（`C-u M-x ecc-focus-project`）を付けると、表示するバッファを選べます。
