...
`k` はプロセスのみを停止して記録は残すため、後から `r` で再開できます。セッションを止めるとそのウィンドウとトランスクリプトも失われるため、実行前に確認を求めます。`D` は記録ファイル自体を完全に削除します。`a` と `d` の動作詳細は [プロンプトとトランスクリプト](/emacs-claude-code/ja/features/prompt/) をご覧ください。

`a` と `d` は、応答する前に何に応答するのかを確認します（`ecc-answer-confirm` を `nil` にすると確認を省けます）。また `ecc-answer-exclude-tools`（既定では `Bash`）に挙げられたツールには応答しません。シェルコマンドは、桁幅に切り詰められた要約からではなく、`RET` で開くトランスクリプトで読んでから答えるべきものだからです。サイドバーの `a` と `d` も同じ動作です。
