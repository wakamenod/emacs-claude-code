...
タブは作成順に並ぶため、位置が変わりません。タブを `mouse-1` でクリックすると、ウィンドウがそのセッションに切り替わります。セッションバッファ内では `C-c C-t`（`ecc-switch-session`）でも同様です。`x` ボタンをクリックすると、確認のうえセッションを終了します。ウィンドウはその場に残り、閉じたタブの隣（右、右端だった場合は左）のタブに切り替わります。同じフレームの他のウィンドウがすでに表示しているタブは飛ばすので、同じトランスクリプトが 2 つのウィンドウに並ぶことはありません。ウィンドウが閉じるのは、表示するものが何も残らなくなったときだけです。
...
`M-x ecc-focus-project` は、他のプロジェクトのトランスクリプトで埋まったフレームを、1 つのプロジェクトだけの表示にリセットします。何も kill せず、プロセスも停止しません。

この役目は Space が引き継ぎました。プロジェクトごとにタブがあるので、そのタブへ切り替えることが絞り込みそのものです。[Space と worktree](/emacs-claude-code/ja/features/spaces/) をご覧ください。
