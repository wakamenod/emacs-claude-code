...
| キー | コマンド | 動作 |
|---|---|---|
| `c` | `ecc-start` | このバッファのプロジェクトでセッションを開始する |
| `r` | `ecc-resume-menu` | セッションを、会話を分岐させるかどうかを選んで再開する |
| `R` | `ecc-rename-session` | セッションとそのバッファの名前を変更する |
| `v` | `ecc-show-session` | セッションを呼び戻し、プロンプトへ移動する |
...
| `U` | `ecc-usage` | 使用量と上限を表示する |
| `?` | `ecc-menu` | メニューを開く |

メニューはここでは `?` なので、どこからでも `C-c c ?` で開けます。ただ、よく開
くものに 4 打鍵は多い。`ecc-menu` は他と同じ普通のコマンドですから、専用のキー
も与えておくとよいでしょう。
...
`C-'` は GUI 用のキーです。端末は control と記号の組を符号化できないので、端末
の Emacs では `C-c m` のように端末が送れるキーを選んでください。
