...
(defun demo-scene-build ()
  ...
  (setq ecc-use-spaces t
        ...
        ;; The machine's own default may allow edits without asking,
        ;; and then `a' and `d' have nothing to answer.
  ...
  (setq demo-root "/tmp/ecc-demo-sidebar-tail/")
...
(defun demo-point-on-session (name)
  "Put point on the SESSION row called NAME, not the Space of that name.
A session in a worktree is named by its branch since 2026-09-17, so the
Space and the session read the same and a search that takes the first
row lands on the Space (2026-09-18)."
  (demo-point-on name 'session))

(defun demo-point-on (name &optional kind)
...
(provide 'sidebar-tail)
;;; sidebar-tail.el ends here
