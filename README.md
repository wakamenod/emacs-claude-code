...
- **[Spaces and worktrees](https://wakamenod.github.io/emacs-claude-code/features/spaces/):** Every project gets its own tab (`ecc-use-spaces`, on by default). This preserves window layouts, so an arrangement returns the way it was left. Going to a project starts a session there when nothing is running (`ecc-space-always-session`, which also closes a Space when its last session ends). `/resume` continues that window with a previously recorded conversation. Opening a worktree opens the repository it came from behind it, and closing a repository closes the worktrees listed under it. A sidebar on the left lists every project and session along with what each is doing. `ecc-start-worktree` checks out a branch beside the repository and starts a session there. The tab bar itself is controlled by `tab-bar-show`: set it to `nil` and Spaces work exactly as they do with the tab bar on screen, with the sidebar listing them.
