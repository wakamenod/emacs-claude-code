...
- **[Spaces and worktrees](https://wakamenod.github.io/emacs-claude-code/features/spaces/):** every project gets a tab of its own -- `ecc-use-spaces`, on by default -- which leaves the windows inside it alone, so an arrangement comes back the way it was left. Going to a project starts a session there when nothing is running -- `ecc-space-always-session`, which also closes a Space once its last session goes -- and `/resume` carries that window on with a conversation recorded earlier. Opening a worktree opens the repository it came from behind it, and closing a repository closes the worktrees drawn under it. A sidebar down the left lists every project and session with what each is doing, and `ecc-start-worktree` checks a branch out beside the repository and starts a session there. The tab bar itself is left to `tab-bar-show`: set it to `nil` and the Spaces work exactly as they do with the strip on screen, the sidebar being the list of them.
